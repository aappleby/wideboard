...
export class Texture {
    ...
    load(url) {
        ...
        image.onload = () => {
            ...
            if (self.format == gl.LUMINANCE) {
                ...
                gl.texImage2D(gl.TEXTURE_2D, 0, gl.LUMINANCE, 256, 1, 0, gl.LUMINANCE, gl.UNSIGNED_BYTE, new Uint8Array(proxy));
            }
            else {
                gl.bindTexture(gl.TEXTURE_2D, self.handle);
    ...
    makeChecker() {
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.handle);
        gl.texImage2D(gl.TEXTURE_2D, 0, this.format, this.width, this.height, 0, this.format, gl.UNSIGNED_BYTE, new Uint8Array(data.buffer));
        this.ready = true;
    }
    ;
    makeNoise() {
...
;
//# sourceMappingURL=wb-texture.js.map