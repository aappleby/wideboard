...
export class App {
    ...
    beginFrame() {
        ...
        if (!this.simpleShader)
            return false;
        if (!this.simpleShader.ready)
            return false;
        if (!this.texShader)
            return false;
        if (!this.texShader.ready)
            return false;
    ...
    render() {
        ...
        this.uniforms['worldToView'].set(-view.origin.x, -view.origin.y, view.scale, view.scale);
        this.grid.draw();
        ...
        if (this.posBuffer && this.texBuffer && this.indexBuffer) {
            ...
            this.uniforms['modelToWorld'].set(0, -linemap.height - 100, linemap.width, linemap.height);
            shader.setUniforms();
